...
function Footer() {
    ...
    const onEmailInpHandler = (value) => {
        setEmailInputValue(value)
    }

    const handleValidation = () => {
        const emailValid = validators.emailValidator(emailInputValue);
        setFormIsValid(emailValid);
    };
    ...
    const handleAddNewsletter = (e) => {
        ...
        fetch(`http://localhost:4000/v1/newsletters`, {
            ...
            .then(res => {
                if (res.ok) {
                    toast.success("ایمیل شما خبرنامه ثبت شد", {
                        position: "top-right",
                        autoClose: 3000,
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-12">
                                        ...
                                        <form action="#" className="footer-widgets__form">
                                            <Input element="input" className={`footer-widgets__form ${FormIsValid ? 'success' : 'error'}`} type="text" value={emailInputValue} onChange={(value) => { onEmailInpHandler(value) }} placeholder='ایمیل خود را وارد کنید' validations={[validators.emailValidator()]} />
                                            <button type="submit" className="footer-widgets__btn" disabled={!FormIsValid} onClick={(e) => { handleAddNewsletter(e) }} >
                                                عضویت
                                            </button>
                                        </form>
                                    </div>
