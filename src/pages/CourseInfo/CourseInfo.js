...
function CourseInfo() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:4000/v1/courses/related/${courseName}`)
      .then((res) => res.json())
      .then((allRelatedCourses) => {
        setRelatedCourses(allRelatedCourses);
        console.log(allRelatedCourses);
      });
  }, [courseName]);
  ...
  const registerCourse = (courseID, price) => {
    if (price === 0) {
      swal({
        icon: "warning",
              ...
              fetch(`http://localhost:4000/v1/offs/${code}`, {
                ...
                .then((res) => {
                  if (res.status == 404) {
                    swal({
                      icon: "error",
                      title: "این کد برای این دوره نامعتبره",
                      button: "باشه",
                    });
                ...
                .then((result) => {
                  fetch(
                    `http://localhost:4000/v1/courses/${courseID}/register`,
                    {
                      ...
                      body: JSON.stringify({
                        price: price - (price * result.percent) / 100,
                      }),
                    }
                  ).then((res) => {
                    if (res.ok) {
  ...
  if (loading) {
    return (
      <>
          ...
          links={[
            {
              id: 1,
              title: (
                <Skeleton width="250px" height="15px" baseColor="#cecece" />
              ),
              to: "",
            },
          ]}
        ...
        <div className="main" style={{ marginBottom: "15px" }}>
          <div className="container">
            <div className="row">
              <div className="col-8">
              ...
              <div className="col-4">
                <div className="courses-info">
                  <div className="course-info">
                    <Skeleton height={60} borderRadius="7px" />
                  </div>
                </div>
              </div>
            </div>
    ...
    <>
      ...
      <Breadcrumb
        links={[
          ...
          { id: 3, title: courseData?.name, to: "/course-info/node-expert" },
        ]}
      />
          ...
          <div className="row">
            <div className="col-6">
              <a href="#" className="course-info__link">
                {courseData.categoryID.title}
              </a>
              <h1 className="course-info__title">{courseData.name}</h1>
              <p className="course-info__text">{courseData.description}</p>
              <div className="course-info__social-media">
                <a href="#" className="course-info__social-media-item">
                  <i className="fab fa-telegram-plane course-info__icon"></i>
                </a>
            ...
            <div className="col-6">
              <video
                src=""
                poster={
                  "http://localhost:4000/courses/covers/" + courseData.cover
                }
          ...
          <div className="row">
            <div className="col-8">
              <div className="course">
                ...
                <div className="course-boxes">
                  <div className="row">
                    <CourseDetailBox
                      title="وضعیت دوره"
                      desc={
                        courseData.isComplete == 1 ? "تکمیل" : "در حال برگزاری"
                      }
                      icon="graduation-cap"
                    />
                    <CourseDetailBox
                      title="شروع دوره"
                      desc={courseData.createdAt.slice(0, 10)}
                      icon={"clock"}
                    />
                    <CourseDetailBox
                      title="آپدیت دوره"
                      desc={courseData.updatedAt.slice(0, 10)}
                      icon="calendar-alt"
                    />
                  </div>
                </div>
                  ...
                  <div className="introduction__topic">
                    <Accordion defaultActiveKey={["0"]} alwaysOpen>
                      <Accordion.Item eventKey="0" className="accordion">
                        <Accordion.Header>قسمت ها</Accordion.Header>
                        {sessions.map((session, index) => (
                          <Accordion.Body>
                            {session.free == 1 ? (
                              ...
                              >
                                <div className="introduction__accordion-body">
                                  <div className="introduction__accordion-right">
                                    ...
                                    <Link
                                      to={`/${courseData.shortName}/${session._id}`}
                ...
                <CommentInput courseName={courseName} comments={comments} />
              </div>
            </div>
              ...
              <div className="courses-info">
                <div className="course-info">
                  {courseData.isUserRegisteredToThisCourse ? (
                    ...
                    <div
                      className="course-info__register pointer"
                      onClick={() => {
                        registerCourse(courseData._id, courseData.price);
                      }}
                    >
                      <span className="course-info__register-title">
                        افزودن به سبد خرید
                    ...
                    <div className="course-info__bottom">
                      <div className="course-info__total-comment">
                        ...
                        <span className="course-info__total-comment-text">
                          {courseData.comments.length} دیدگاه
                        </span>
                      </div>
                ...
                <div className="course-info">
                  ...
                  <span className="course-info__topic-text">
                    <span>برای مشاهده و یا دانلود دوره روی کلمه</span>
                    ...
                    <ul className="course-info__courses-list">
                      {relatedCourses.map((relatedCourse) => (
                        <li className="course-info__courses-list-item">
                          <Link
                            to={`/course-info/${relatedCourse.shortName}`}
                            className="course-info__courses-link"
                          >
                            <img
                              src={`http://localhost:4000/courses/covers/${relatedCourse.cover}`}
                              alt="Course Cover"
